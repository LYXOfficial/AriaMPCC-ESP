...
int CalendarPage::getFirstDayOfWeek(int year, int month) {
  ...
  int k = year % 100; int j = year / 100; int q = 1;
...
String CalendarPage::lunarDayToChinese(int d) {
  ...
  if (d == 10) return String("初十");
  if (d < 10) return String("初") + String(digits[d]);
  if (d < 20) { if (d == 20) return String("二十"); return String("十") + String(digits[d - 10]); }
  if (d == 20) return String("二十");
  if (d < 30) return String("二十") + String(digits[d - 20]);
  return String("三十");
}
String CalendarPage::getLunarDate(int y, int m, int d) {
  if (y < 1900 || y > 2100) {
    return String("农历") + String(y) + String("年") + String(m) + String("月") + String(d) + String("日");
  }
  Solar s{y, m, d};
  Lunar L = SolarToLunar(s);
  static const char *stems[] = {"甲","乙","丙","丁","戊","己","庚","辛","壬","癸"};
  static const char *branches[] = {"子","丑","寅","卯","辰","巳","午","未","申","酉","戌","亥"};
  int idxStem = (L.year - 4) % 10; if (idxStem < 0) idxStem += 10;
  int idxBranch = (L.year - 4) % 12; if (idxBranch < 0) idxBranch += 12;
  String gzYear = String(stems[idxStem]) + String(branches[idxBranch]);
  static const char *lunarMonthNamesChinese[] = {"正月","二月","三月","四月","五月","六月","七月","八月","九月","十月","冬月","腊月"};
  String monthName = ""; int mi = L.month - 1;
  if (mi >= 0 && mi < 12) monthName = String((L.isLeap ? "闰" : "")) + String(lunarMonthNamesChinese[mi]);
  else monthName = String(L.month) + "月";
  String dayName = lunarDayToChinese(L.day);
  return gzYear + String("年") + monthName + dayName;
}

void CalendarPage::render(bool full) {
  if (full) { refreshInProgress = true; display.setFullWindow(); }
  else { display.setPartialWindow(0, 0, display.width(), display.height()); }
  display.firstPage();
  do {
    display.fillScreen(GxEPD_WHITE);
    int leftX = 5; int startY = 35;
    ...
    int calendarX = 110; int calendarY = 15; int cellW = 14; int cellH = 12;
    ...
    for (int i = 0; i < 7; i++) { u8g2Fonts.setCursor(calendarX + i * cellW, headerY); u8g2Fonts.print(weekHeader[i]); }
    ...
    for (int week = 0; week < 6; week++) {
      for (int day = 0; day < 7; day++) {
        ...
        if (dayNum > 0 && dayNum <= daysInMonth) {
          int cellX = calendarX + day * cellW; int cellY = gridStartY + week * cellH;
    ...
    time_t rawtime = timeClient.getEpochTime();
    struct tm *timeinfo = localtime(&rawtime);
    int year = timeinfo->tm_year + 1900; int month = timeinfo->tm_mon + 1; int day = timeinfo->tm_mday; int weekday = timeinfo->tm_wday;
    int hours = timeClient.getHours(); int minutes = timeClient.getMinutes();
    String currentTimeStr = String(year) + "-" + String(month < 10 ? "0" : "") + String(month) + "-" + String(day < 10 ? "0" : "") + String(day) + " " + String(weekDaysChinese[weekday]) + " " + String(hours < 10 ? "0" : "") + String(hours) + ":" + String(minutes < 10 ? "0" : "") + String(minutes);
    int timeW = u8g2Fonts.getUTF8Width(currentTimeStr.c_str()); int timeX = (display.width() - timeW) / 2 - 20; int timeY = dividerY + 15;
    u8g2Fonts.setCursor(timeX, timeY); u8g2Fonts.print(currentTimeStr);
    u8g2Fonts.setCursor(5, dividerY + 15); u8g2Fonts.print("< 主页");
    u8g2Fonts.setCursor(172, dividerY + 15); u8g2Fonts.print("闹钟 >");
...
void CalendarPage::onRight() {
  ...
  switchPageAndFullRefresh(next);
    ...
    selectedYear = std::min(selectedYear + 1, 2030);
    selectedDay = std::min(selectedDay, getDaysInMonth(selectedYear, selectedMonth));
    render(false);
  } else if (currentCursor == CURSOR_MONTH) {
    selectedMonth++; if (selectedMonth > 12) { selectedMonth = 1; selectedYear = std::min(selectedYear + 1, 2030); }
    selectedDay = std::min(selectedDay, getDaysInMonth(selectedYear, selectedMonth));
    render(false);
  } else if (currentCursor == CURSOR_DAY) {
    selectedDay++; int maxDay = getDaysInMonth(selectedYear, selectedMonth);
    if (selectedDay > maxDay) { selectedDay = 1; selectedMonth++; if (selectedMonth > 12) { selectedMonth = 1; selectedYear = std::min(selectedYear + 1, 2030); } }
    render(false);
  }
}
...
void CalendarPage::onLeft() {
  ...
  switchPageAndFullRefresh(prev);
    ...
    selectedYear = std::max(selectedYear - 1, 2020);
    selectedDay = std::min(selectedDay, getDaysInMonth(selectedYear, selectedMonth));
    render(false);
  } else if (currentCursor == CURSOR_MONTH) {
    selectedMonth--; if (selectedMonth < 1) { selectedMonth = 12; selectedYear = std::max(selectedYear - 1, 2020); }
    selectedDay = std::min(selectedDay, getDaysInMonth(selectedYear, selectedMonth));
    render(false);
  } else if (currentCursor == CURSOR_DAY) {
    selectedDay--; if (selectedDay < 1) { selectedMonth--; if (selectedMonth < 1) { selectedMonth = 12; selectedYear = std::max(selectedYear - 1, 2020); } selectedDay = getDaysInMonth(selectedYear, selectedMonth); }
    render(false);
  }
}
